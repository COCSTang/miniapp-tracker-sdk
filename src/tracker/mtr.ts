import {logger, logInfo,dateFormat,_encodeStr,_formatExinfoParam,getMainInfo,formatSeed,dealExtra } from "../utils/common"
...
export class MTR implements Event  {
  ...
  version:string = "" //mini app version
  ...
  stat_app_launch: boolean=true
  stat_app_show: boolean = true
...
* */
  _packFinalData(data:RemoteLogType) {
    ...
    //const type = "MINI";
    if(!this.appId && my.canIUse("getAppIdSync") &&  my.getAppIdSync) {
...
*/
  ...
  log(r:string,param:any={}) {
    ...
    param4.refAction = this.refAction
    my.reportAnalytics(param2, param4);
    return this._remoteLog({
  ...
  api(param:API) {
    ...
    let myUrl =getMainInfo().url;
    let param2 =bizSuccess?(time>30000?"API_UNUSUAL":"API_SLOW"):(success?"API_BIZ_FAIL":"API_FAIL") //this.formatSeed(api)    
    my.reportAnalytics(param2.toLowerCase(), {api,success,bizSuccess,time,code, msg, response});
    return this._remoteLog({
